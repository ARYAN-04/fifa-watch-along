...
│   │   ├── components/  # Dashboard UI components (Dashboard, WinProbChart, EventTicker, MatchInfo, PlayerLineup)
...
│   └── package.json     # Deps: next 16, react 19, recharts 3, lucide-react, tailwindcss 4
...
| 2026-06-10 | **Phase 4 — Frontend dashboard implemented.** Installed `lucide-react` + `tailwindcss` v4 + `@tailwindcss/postcss`. Set up Tailwind v4 entry point in `globals.css` and PostCSS config. Updated metadata in `layout.tsx`. Created 5 components: `Dashboard.tsx` (orchestrator with 30s polling and loading/error/no-match states), `WinProbChart.tsx` (Recharts AreaChart with gradient fill, tooltip, reference line), `EventTicker.tsx` (color-coded event feed with type dots), `MatchInfo.tsx` (scoreboard with team badges and ELO), `PlayerLineup.tsx` (top 11 players by rating per team). Replaced default Next.js starter page with Dashboard. Removed `page.module.css`. All child components are typed against Django API response shapes. Build verified. |
| 2026-06-11 | **Phase 4 refinements.** Added mock data fallback to 4 BFF proxy routes (match, win-probability, events, players) so the frontend works standalone without a Django backend. Sets are hardcoded Argentina vs Canada with realistic player ratings. Fixed white background gaps on scroll by setting `html, body { background-color: #09090b }` in Tailwind config. Added dark-styled `::-webkit-scrollbar` rules (zinc-900 track, zinc-700 thumb) to prevent ugly white scrollbar flash. Suppressed Recharts SSR hydration width/height warning with `minWidth={0}` on `ResponsiveContainer`. Added `.scrollbar-hide` utility to hide EventTicker scrollbar while preserving scroll. Removed LIVE badge from dashboard header. Updated all three docs (AGENTS.md, README.md, implementation-notes.md). |
