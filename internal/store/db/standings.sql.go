...
const getStandingsByCompetition = `-- name: GetStandingsByCompetition :many
WITH finished AS (
...
type GetStandingsByCompetitionRow struct {
	...
	GoalDiff     int64          `json:"goal_diff"`
	Points       int64          `json:"points"`
}

func (q *Queries) GetStandingsByCompetition(ctx context.Context, competitionID int64) ([]GetStandingsByCompetitionRow, error) {
	rows, err := q.db.QueryContext(ctx, getStandingsByCompetition, competitionID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []GetStandingsByCompetitionRow{}
	for rows.Next() {
		var i GetStandingsByCompetitionRow
		if err := rows.Scan(
			&i.TeamID,
			&i.TeamName,
			&i.ShortName,
